from tkinter import END, Tk, Label, Button, Text, Scrollbar, RIGHT, Y
...
class mainGUI():
    ...
    def __init__(self, master):
        ...
        T = Text(master, height = 1, width = 25)
        ...
        T2 = Text(master, height = 1, width = 25)
        T2.insert(END, "Last Name")
        # T.bind('<KeyRelease>',self.my_upd)
        self.lname = T2


        self.gen_file_button = Button(master, text="Update", command=self.gen_file)

        
        self.label = Label(master, text="This is our first GUI!")

        l3 = Label(root, text = "No path selected", bg = "white")
        # T3.insert(END, "File Path")
        self.path = l3

        self.choose_path_button = Button(master, text="Choose Path", command=self.choose_path)
        ...
        self.close_button.pack(padx=padding, pady=padding+10)
    ...
    def my_upd(self, value):
        my_str= self.fname.get("1.0",'end-1c') #The input string except the last line break
        ...
        if(char_numbers > 7):
            self.fname.delete('end-{}c'.format(char_numbers-7)) # remove last char of text widget
